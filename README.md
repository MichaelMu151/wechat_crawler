...
- 后端稳定性：历史列表抓取在单个微信会话失效、过期或触发风控类错误时，会在同一页自动尝试切换到下一个可用会话；只有所有会话都失败后，才将账号标记为需要更新会话或失败。
- 任务控制：Web 后台任务增加轻量心跳与安全取消检查，长任务会周期性更新 `heartbeat_at`，并在账号或文章处理边界响应取消请求，减少任务“点了取消但还一直跑”的情况。
- 启动恢复：API 启动时不再把所有 `running` 任务立即判定为过期，而是使用合理的过期阈值，避免正常长任务被误标失败。
- Web 管理台：采集任务按钮和公开链接归档增加提交中状态、禁用状态和错误提示，避免重复点击或静默失败。
- 文章库浏览：新增搜索防抖、状态筛选、总数显示、上一页/下一页分页，以及失败原因展示，方便排查 `failed`、`retry_wait` 等文章状态。
- 状态展示：常见文章和任务状态增加中文显示，例如“已归档”“待抓取”“等待重试”“运行中”“完成”等。
- 测试验证：新增历史会话切换、任务心跳/取消、启动恢复阈值相关测试；当前 `wechat_archive/tests` 测试通过，前端 `npm run build` 通过。
